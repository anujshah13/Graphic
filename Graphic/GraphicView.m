...
- (id)initWithFrame:(CGRect)frame
{
	...
	//	self.bounds = CGRectMake(-w / 2, -h / 2, w, h);
        ...
        NSDateComponents *components = [calendar components:(NSHourCalendarUnit|NSMinuteCalendarUnit|NSSecondCalendarUnit) fromDate:dateValue];
        
        angleMinute = [components minute];
        angleHours = [components hour];
        angleSeconds = [components second];
        
        if(angleHours > 12)
            angleHours -= 12;
        
        NSLog(@"SMH %d %d %d", angleSeconds, angleMinute, angleHours);
    }
    return self;
...
- (void)drawRect:(CGRect)rect
{
  ...
  //  CGFloat w = self.bounds.size.width;
    ...
    CGFloat angle1 = DEG2RAD(210);
    CGFloat angle2 = DEG2RAD(360);
    CGFloat angle3 = DEG2RAD(90);
    CGFloat initialAngle = DEG2RAD(90);
